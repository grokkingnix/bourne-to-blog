...
You can make use of [openring](https://git.sr.ht/~sircmpwn/openring) with the `-o` flag when building your posts. You need the additional `openring.sh` file from the `modules` folder. Configure the variable values and you can add the flag to your regular post build/publish workflow i.e. `blog -bosp post-file.md`. You can find more details on this [post](https://nixing.mx/posts/integrating-openring-into-a-blog.html).
